...
CREATE TABLE StageRequest (requestDesc VARCHAR2(2048), castorReqId VARCHAR2(2048), overwriteOption NUMBER, protocol VARCHAR2(2048), euid NUMBER, egid NUMBER, lastCheck INTEGER, nextCheck INTEGER, proxyCert CLOB, removeSourceFiles NUMBER, rhHost VARCHAR2(2048), rhPort NUMBER, creationTime INTEGER, svcClass VARCHAR2(2048), srmRequestId VARCHAR2(2048), handler NUMBER, id INTEGER CONSTRAINT PK_StageRequest_Id PRIMARY KEY, srmUser INTEGER, status INTEGER, requestType INTEGER) INITRANS 50 PCTFREE 50 ENABLE ROW MOVEMENT;
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('StageRequest', 'status', 3, 'REQUEST_POLLING');
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('UserFile', 'status', 0, 'FILE_PENDING');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('UserFile', 'status', 1, 'FILE_STAGED');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('UserFile', 'status', 2, 'FILE_GCCANDIDATE');
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('Pin', 'status', 0, 'PIN_NEW');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('Pin', 'status', 1, 'PIN_PINNED');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('Pin', 'status', 2, 'PIN_EXTENDED');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('Pin', 'status', 3, 'PIN_RELEASED');
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('Pin', 'status', 4, 'PIN_EXPIRED');

/* SQL statements for type SubRequest */
CREATE TABLE SubRequest (spaceToken VARCHAR2(2048), turl VARCHAR2(2048), expirationInterval INTEGER, castorFileName VARCHAR2(2048), reservedSize INTEGER, reason VARCHAR2(2048), aborted NUMBER, errorCode NUMBER, surl VARCHAR2(2048), id INTEGER CONSTRAINT PK_SubRequest_Id PRIMARY KEY, request INTEGER, status INTEGER) INITRANS 50 PCTFREE 50 ENABLE ROW MOVEMENT;
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('SubRequest', 'status', 5, 'SUBREQUEST_TURLSENT');
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('CopySubRequest', 'remoteStatus', 5, 'SUBREQUEST_TURLSENT');
...
INSERT INTO ObjStatus (object, field, statusCode, statusName) VALUES ('CopySubRequest', 'localStatus', 5, 'SUBREQUEST_TURLSENT');
...
INSERT INTO CastorVersion VALUES ('-', '2_8_0');

/*******************************************************************
 *
 * @(#)RCSfile: oracleTrailer.sql,v  Revision: 1.50  Release Date: 2009/07/03 15:25:37  Author: itglp 
 *
...
UPDATE CastorVersion SET schemaVersion = '2_8_0';
...
CREATE GLOBAL TEMPORARY TABLE CleanupReqHelper
  (id NUMBER CONSRTAINT NN_CRH_Id NOT NULL, creationTime NUMBER CONSTRAINT NN_CRH_CreationTime NOT NULL)
...
/* Rename the autogenerated object for the LOB entity */
ALTER TABLE StageRequest MOVE LOB (proxyCert) STORE AS "LOB_StageRequest_ProxyCert";

/* Add constraints */
ALTER TABLE StageRequest MODIFY (status CONSTRAINT NN_StageRequest_status NOT NULL);
ALTER TABLE SubRequest MODIFY (status CONSTRAINT NN_SubRequest_status NOT NULL, castorFileName CONSTRAINT NN_SubRequest_CFN NOT NULL);

ALTER TABLE SrmUser ADD CONSTRAINT U_SrmUser_vo_dn_userId UNIQUE (vo, dn, userId);
ALTER TABLE SrmUser MODIFY (VO CONSTRAINT NN_SrmUser_VO NOT NULL);

ALTER TABLE StorageArea ADD CONSTRAINT U_StorageArea_srmUser_spToken UNIQUE (srmUser, spaceToken);
ALTER TABLE StorageArea MODIFY (srmUser CONSTRAINT NN_StorageArea_srmUser NOT NULL);
...
CREATE INDEX I_SubRequest_CastorFileName ON SubRequest (castorFileName);
...
CREATE INDEX I_StageRequest_Handler_CT_Id ON StageRequest (handler, creationTime, id); 
...
CREATE OR REPLACE PROCEDURE requestToDo(inHandler IN INTEGER, rId OUT INTEGER) AS
...
BEGIN
  ...
  LOOP   -- loop over all possible candidates
    ...
    BEGIN
      ...
      SELECT id INTO intRId FROM StageRequest
       WHERE id = intRId AND handler = inHandler
      FOR UPDATE NOWAIT;
      -- we got a valid one, update and exit the loop
