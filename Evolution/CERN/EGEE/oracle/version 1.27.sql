...
CREATE INDEX channel_channel_state ON t_channel(channel_state) TABLESPACE &1;

-- t_channel_vo_share indexes:
-- t_channel_vo_share(channel_name,vo_name) is primary key
CREATE INDEX channel_vo_share_channel_name ON t_channel_vo_share(channel_name) TABLESPACE &1; 

-- t_channel_acl indexes:
-- t_channel_acl(channel_name,principal) is primary key
CREATE INDEX channel_acl_channel_name ON t_channel_acl(channel_name) TABLESPACE &1;

-- t_vo_acl indexes:
-- t_vo_acl(vo_name,principal) is primary key
CREATE INDEX vo_acl_vo_name ON t_vo_acl(vo_name) TABLESPACE &1;
...
CREATE INDEX job_vo_name      ON t_job(vo_name)      TABLESPACE &1;
...
CREATE INDEX transfer_request_id        ON t_transfer(request_id)     TABLESPACE &1;
CREATE INDEX transfer_file_id           ON t_transfer(file_id)        TABLESPACE &1;
CREATE INDEX transfer_transfer_state    ON t_transfer(transfer_state) TABLESPACE &1;

-- t_trace indexes:
-- t_trace(trace_id) is primary key
CREATE INDEX trace_transfer_unique_id ON t_trace(request_id,file_id) TABLESPACE &1;
