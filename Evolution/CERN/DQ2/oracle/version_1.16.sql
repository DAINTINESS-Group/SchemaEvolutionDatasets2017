...
CREATE INDEX idx2_${PREFIX}t_dataset_request   ON  ${PREFIX}t_dataset_request(dsn) LOCAL
...
CREATE TABLE ${PREFIX}t_file_replica (
	  ...
	  CONSTRAINT  ${PREFIX}pk_file_replica PRIMARY KEY (locationId, creationdate, fileId)
)
PARTITION BY LIST (locationId) 
(
	 PARTITION partition_1    VALUES (1), 
...
CREATE UNIQUE INDEX ${PREFIX}idx_file_replica ON ${PREFIX}t_file_replica(locationId, fileId);


-- Function-Based Indexes

DROP INDEX idx_${PREFIX}_t_file_replica 


CREATE INDEX idx1_v1_t_file_replica  ON  v1_t_file_replica
(
  decode (requestState, 0,0, 1,1, 2,2 ,NULL),   
  decode (requestState, 0,locationId, 1,locationId, 2,locationId ,NULL)
)
COMPRESS 2 online	
COMPUTE STATISTICS
LOCAL

CREATE INDEX idx11_v1_t_file_replica  ON  v1_t_file_replica
(
  decode (requestState, 3,locationId, 4,locationId ,NULL),
  decode (requestState, 3,3, 4,4 ,NULL)
)
COMPRESS 2 online	
COMPUTE STATISTICS
...
CREATE INDEX idx11_v1_t_file_replica  ON  v1_t_file_replica
(
	decode (requestState, NULL, NULL, locationId),
	decode (requestState, NULL, NULL, requesttype),
	requestState
)
online	
COMPUTE STATISTICS
...
analyze 
table  v1_t_file_replica
compute statistics 
for columns requestState, locationId


CREATE INDEX idx3_v1_t_file_replica  ON  v1_t_file_replica
   ( decode (requestState, 0,0, 1,1, 2,2 ,NULL), 
     decode (requestState, 0,locationId, 1,locationId, 2,locationId ,NULL),
     decode (requestState, 0,fileId, 1,fileId, 2,fileId ,NULL)
   ) 
---   COMPRESS 3 online;


CREATE INDEX idx4_v1_t_file_replica  ON  v1_t_file_replica (locationId,creationdate, fileId)
COMPRESS 2 online
	

___ bitmap join indexes

-- v1_t_dataset_request.locationId,
-- v1_t_dataset_request.duid,
...
CREATE INDEX idx6_${PREFIX}_t_file_replica  ON  ${PREFIX}_t_file_replica
	(decode (requestState, 0,0, 1,1, 2,2 ,NULL), decode (requestState, 0,locationId, 1,locationId, 2,locationId ,NULL))
COMPUTE STATISTICS

CREATE INDEX idx8_${PREFIX}t_file_replica  ON  ${PREFIX}t_file_replica
	(decode (requestState, 0,0, 1,1, 2,2 ,NULL), decode (requestState, 0,locationId, 1,locationId, 2,locationId ,NULL))
COMPUTE STATISTICS LOCAL;

CREATE INDEX idx2_v1_t_file_replica  ON  v1_t_file_replica
	decode (requestState, 0,0, 1,1, 2,2 ,NULL)
COMPUTE STATISTICS LOCAL


CREATE INDEX idx10_v1_t_file_replica  ON  v1_t_file_replica(locationId)

CREATE INDEX idx12_v1_t_file_replica  ON  v1_t_file_replica
	decode (requestState, 0,locationId ,NULL))
COMPUTE STATISTICS
	

CREATE INDEX idx12_v1_t_file_replica  ON  v1_t_file_replica
(CASE
 WHEN requestState=0 THEN locationId
 WHEN requestState=1 THEN  locationId
 WHEN requestState=2 THEN  locationId
 ELSE NULL END)
COMPUTE STATISTICS



CREATE INDEX idx3_${PREFIX}t_file_replica  ON  ${PREFIX}t_file_replica
(CASE
 WHEN requestState=0 THEN 
 WHEN requestState=1 THEN  locationId
 WHEN requestState=2 THEN  locationId
 ELSE NULL END)
COMPUTE STATISTICS;
