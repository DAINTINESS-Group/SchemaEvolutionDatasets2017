...
CREATE TABLE StageRequest (requestDesc VARCHAR2(2048), expirationTime INTEGER, expirationInterval NUMBER, castorReqId VARCHAR2(2048), lifetimeUpdates NUMBER, overwriteOption NUMBER, protocol VARCHAR2(2048), euid NUMBER, egid NUMBER, id INTEGER PRIMARY KEY, srmUser INTEGER, requestType INTEGER, status INTEGER) INITRANS 50 PCTFREE 50;
...
CREATE TABLE Pin (expirationTime INTEGER, id INTEGER PRIMARY KEY, userFile INTEGER, srmUser INTEGER, request INTEGER, status INTEGER) INITRANS 50 PCTFREE 50;

/* SQL statements for type SubRequest */
CREATE TABLE SubRequest (spaceToken VARCHAR2(2048), turl VARCHAR2(2048), id INTEGER PRIMARY KEY, request INTEGER, userFile INTEGER, status INTEGER) INITRANS 50 PCTFREE 50;
...
/* PL/SQL method to delete expired file pins and the related files&requests if needed */
CREATE OR REPLACE PROCEDURE deleteExpiredPins() AS
BEGIN
END;
