...
/*******************************************************************
 *
 * @(#)$RCSfile: castor_oracle_create.sql,v $ $Revision: 1.74.4.5 $ $Release$ $Date: 2007/04/23 08:56:56 $ $Author: itglp $
 *
...
INSERT INTO CastorVersion VALUES ('2_1_2_4', '$Revision: 1.74.4.5 $ $Date: 2007/04/23 08:56:56 $');
...
/******************************************************
 * All hotfixes of version v2.1.2-4/9 are appended here
 ******************************************************/

/* we force the plsqlrevision to be the same as the hotfixes which are appended here */
UPDATE CastorVersion SET plsqlrevision = 'Revision: 1.365.4, Date: 2007/04/23';


/* improved, even more optimized stager_qry */
CREATE OR REPLACE PROCEDURE internalStageQuery
 (cfs IN "numList",
  svcClassId IN NUMBER,
  result OUT castor.QueryLine_Cur) AS
...
/* Fixes around the support for update requests. Not optimal yet */
...
BEGIN
 ...
 EXCEPTION WHEN NO_DATA_FOUND THEN
   ...
   contextPIPP := 2;
 END;
 -- get uid, gid, reserved space, and svcClass from Request
...
BEGIN
  ...
  -- get current SvcClass for the Put/Update
  -- a PutDone is ignored here
  ...
  END;
  EXCEPTION WHEN NO_DATA_FOUND THEN NULL;
  -- For a failed PutDone nothing else is needed
END;
  ...
  IF reqType = 39 THEN -- PutDone
    ...
    IF stat.COUNT > 0 THEN
      ...
      EXCEPTION WHEN NO_DATA_FOUND THEN
        ...
        OPEN sources
          FOR SELECT DiskCopy.id, DiskCopy.path, DiskCopy.status,
                     FileSystem.weight, FileSystem.mountPoint, DiskServer.name
                FROM DiskCopy, SubRequest, FileSystem, DiskServer, DiskPool2SvcClass
               WHERE SubRequest.id = rsubreqId
                 AND SubRequest.castorfile = DiskCopy.castorfile
                 AND FileSystem.diskpool = DiskPool2SvcClass.parent
