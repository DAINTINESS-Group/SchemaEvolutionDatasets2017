...
INSERT INTO CastorVersion VALUES ('-', '2_1_4_10');

/*******************************************************************
 *
 * @(#)RCSfile: oracleTrailer.sql,v  Revision: 1.502.2.21  Date: 2007/10/24 09:57:08  Author: waldron 
 *
...
BEGIN
  ...
  IF reqType = 36 OR reqType = 38 THEN
    UPDATE SubRequest 
       SET status = 8, lastModificationTime = getTime()
...
/* PL/SQL method to change the status of all streams given by id */
CREATE OR REPLACE PROCEDURE changeStreamsStatus(streamIds IN castor."cnumList", 
                                                newStatus IN NUMBER) AS 
BEGIN 
  FOR i in streamIds.FIRST .. streamIds.LAST 
  LOOP 
    UPDATE Stream set Status = newStatus where id = streamIds(i); 
  END LOOP; 
  COMMIT; 
END; 
