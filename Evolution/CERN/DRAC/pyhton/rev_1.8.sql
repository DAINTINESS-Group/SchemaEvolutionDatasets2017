...
-- ALTER  TABLE JobStatus ADD PRIMARY KEY(JobID, Name);
    ...
    Value BLOB  not null,
    FOREIGN KEY(JobID) REFERENCES Job(JobID)
CREATE TABLE LogInfos (JobID integer not null,
                                           Event varchar(100) not null,
                                           Date Date,
                                           Time time);
ALTER  TABLE LogInfos ADD FOREIGN KEY(JobID) REFERENCES Job(JobID);
    Event varchar(100) not null,
    Date Date,
    Time time,
    FOREIGN KEY(JobID) REFERENCES Job(JobID)
...
CREATE TABLE ISandbox (   JobID  integer not null,
                              File    varchar(100) NOT NULL,
                              Value   BLOB NOT NULL);

ALTER  TABLE ISandbox ADD PRIMARY KEY (JobID, File);
    File   varchar(100) NOT NULL,
DROP TABLE IF EXISTS OSandbox;

CREATE TABLE OSandbox (   JobID  integer not null,
                               File    varchar(100) NOT NULL,
                               Value   BLOB NOT NULL);
    Value  LONGBLOB NOT NULL,
ALTER  TABLE  OSandbox ADD PRIMARY KEY (JobID, File);
--------------------------------------------------------------------------------
    Type  enum  ('ascii', 'binary') not null,
);
CREATE TABLE SandboxReady ( JobID  integer not null,
                            RetrievedOutput enum ('True', 'False') not null,
                            OutputDate Date,
                            OutputTime time);

ALTER  TABLE SandboxReady ADD PRIMARY KEY (JobID);
