...
GRANT SELECT,INSERT,LOCK TABLES,UPDATE,DELETE,CREATE,DROP,ALTER ON perftest.* TO Ian IDENTIFIED BY 'Ian';
-- DELETE FROM user WHERE user="Ian";
...
CREATE TABLE Job (   JobID integer not null auto_increment primary key
                   , JDL BLOB
                   , JobType enum ('Job', 'DAG', 'Reservation', 'Co-allocation') not null
		   ...
		   , SubmissionTime time
                   ...
                   , Status enum        ('submitted', 'running','waiting','ready', 'done', 'rescheduled', 'outputready', 'matched') not null);
...
CREATE TABLE InformationService (Profile varchar(100), Name varchar(100) not null, Value varchar(100) not null);
ALTER  TABLE InformationService ADD PRIMARY KEY (Profile, Name, Value);
